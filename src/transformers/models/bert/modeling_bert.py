...
class BertForSequenceClassification(BertPreTrainedModel):  # a.k.a NER. BertPreTrainedModel 상속.
    def __init__(self, config):
        super().__init__(config)  # parent class의 __init__함수 호출. 매개변수는 config
        self.num_labels = config.num_labels
        self.config = config

        self.bert = BertModel(config)  # Pre train된 model을 가져오는 코드
        classifier_dropout = (
        ...
        self.dropout = nn.Dropout(
            classifier_dropout
        )  # nn. ~는 layer을 선언한다는 것   (Neural Network라고 찍음 ㅋ) Dropout class instance 생성.
        """ 
        Overfitting막는 역할
        """
        self.classifier = nn.Linear(
            config.hidden_size, config.num_labels
        )  # Linear Layer. config.hidden_size : in_features(int), config.num_labels : out_features(int). Linear class instance 생성.
    ...
    def forward(
        # 아직모름. layer 층을 쌓는다는 것으로 알고 있는데 그런 요소가 이 안에 없음.. 보긴 봐야함.. ㄹㅇㅋㅋ
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        token_type_ids: Optional[torch.Tensor] = None,
...
class BertForTokenClassification(BertPreTrainedModel):  # a.k.a NER. BertPreTrainedModel 상속.
    r"""
    ...
    모델의 모든 매개변수가 있는 모델 구성 클래스입니다. 구성 파일로 초기화하면 모델과 관련된 가중치가 로드되지 않고 구성만 로드됩니다. 모델 가중치를 로드하려면 from_pretrained() 메서드를 확인하세요.
    """
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def forward(
        ...
        loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()  # 2 번 레이어는 BCE로 해야함.
            loss_fct2 = BCEWithLogitsLoss()  # 내가추가한코드. 2번 layer에 대한 loss
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))  # 원본코드.
            loss2 = loss_fct2(
                logits2.view(-1, self.num_labels), labels.view(-1)
            )  # 내가추가한코드. loss2 계산
